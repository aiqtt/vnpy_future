...
from vnpy.api.ctplts import MdApi, TdApi, defineDict
from vnpy.trader.vtGateway import *
from vnpy.trader.vtFunction import getJsonPath, getTempPath, convertFloatMaxValue
...
########################################################################
class CtpLTSGateway(VtGateway):
    """CTP接口"""

    #----------------------------------------------------------------------
    def __init__(self, eventEngine, gatewayName='CTP'):
        """Constructor"""
        super(CtpLTSGateway, self).__init__(eventEngine, gatewayName)
...
class CtpMdApi(MdApi):
    ...
    def __init__(self, gateway):
        ...
        self.appID=EMPTY_STRING
        self.authCode=EMPTY_STRING
    ...
    def onRtnDepthMarketData(self, data):
        ...
        tick.lastPrice = convertFloatMaxValue(data['LastPrice'])
        ...
        tick.openPrice = convertFloatMaxValue(data['OpenPrice'])
        tick.highPrice = convertFloatMaxValue(data['HighestPrice'])
        tick.lowPrice = convertFloatMaxValue(data['LowestPrice'])
        tick.preClosePrice = convertFloatMaxValue(data['PreClosePrice'])

        tick.upperLimit = convertFloatMaxValue(data['UpperLimitPrice'])
        tick.lowerLimit = convertFloatMaxValue(data['LowerLimitPrice'])

        # CTP只有一档行情
        tick.bidPrice1 = convertFloatMaxValue(data['BidPrice1'])
        tick.bidVolume1 = data['BidVolume1']
        tick.askPrice1 = convertFloatMaxValue(data['AskPrice1'])
        tick.askVolume1 = data['AskVolume1']

        tick.TradingDay = data['TradingDay']          # 交易日
        tick.PreSettlementPrice = convertFloatMaxValue(data['PreSettlementPrice'])   # 上次结算价
        tick.PreOpenInterest = data['PreOpenInterest']        # 昨持仓量
        tick.ClosePrice = convertFloatMaxValue(data['ClosePrice'])           # 今收盘价
        tick.SettlementPrice = convertFloatMaxValue(data['SettlementPrice'])      # 本次结算价
        tick.AveragePrice = convertFloatMaxValue(data['AveragePrice'])        # 单日均价


        # 大商所日期转换   大商所  ActionDay 是交易日
    ...
    def isDirtyData(self,tick):
        ...
        if ((MORNING_START <= dt < MORNING_END) or
...
class CtpTdApi(TdApi):
    ...
    #----------------------------------------------------------------------
    def connect(self, userID, password, brokerID, address, authCode,appID,userProductInfo):#userProductInfo
        """初始化连接"""
        self.userID = userID                # 账号
        self.password = password            # 密码
        self.brokerID = brokerID            # 经纪商代码
