...
from vnpy.trader.data.mysqlEngine import *
from pymongo.errors import ConnectionFailure

from vnpy.event import Event
from vnpy.trader.vtGlobal import globalSetting, globalSymbolSetting
...
class MainEngine(object):
    ...
    def getSymbolSetting(self,symbol):

        symbolPre  =  re.sub(r'([\d]+)','',symbol)

        return globalSymbolSetting[symbolPre]
    ...
    def dbConnect(self):
        ...
        if not self.mysqlClient:
            try:
                config = {}
                config['host'] = globalSetting["mysqlHost"]
                config['port'] = globalSetting["mysqlPort"]
                config['user'] = globalSetting["mysqlUser"]
                config['passwd'] = globalSetting["mysqlPassword"]
                config['db'] = globalSetting["mysqlDb"]
                config['accountId'] = globalSetting["accountId"]
                config['charset'] = "utf8"
                self.mysqlClient = mysqlManager(config)
                self.writeLog(u"mysql连接成功")

                if self.mysqlClient:
                    self.dataEngine.setMysqlClient(self.mysqlClient)

            except ConnectionFailure:
                self.writeLog(u"mysql连接失败")
...
class LogEngine(object):
    ...
    def processLogEvent(self, event):
        ...
        ##日志写入数据库
        if self.mysqlClient:
            self.mysqlClient.dbInsert(SQL_TABLENAME_LOG, log)

    def processErrorEvent(self,event):
        ##VtErrorData
        log = event.dict_['data']
        function = self.levelFunctionDict[self.LEVEL_INFO]     # 获取日志级别对应的处理函数

        msg = '\t'.join(["errorID:", str(log.errorID)," errorMsg:", log.errorMsg, " additionalInfo:", log.additionalInfo])

        logData = VtLogData()
        logData.gatewayName = "CTP"
        logData.logLevel = self.LEVEL_INFO
        logData.logTime = log.errorTime
        logData.logContent = msg

        function(msg)

        ##日志写入数据库
        #if self.mysqlClient:
            #self.mysqlClient.dbInsert(SQL_TABLENAME_LOG, logData)
